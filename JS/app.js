...
let selectedProject = {};
let projectIndexTracker = -1;
const projectsCollection = [];
const displayMatches = [];
let modalNumber = 0;


// Global listener to address Mac/Chrome 'bug' - prevent cross-origin risk - (so called feature) which interferes with 
// eventlisteners within canvas animation

canvasListener.addEventListener("click", fl_MouseClickHandler.bind(this));
...
let Project = class {
    constructor(projectname, theme, url, repo, thumbnail, description, extended, techniques, lighthouse, colorCode) {
...
let getshot = new Project("getshot", "Portfolio",
    ...
    `,
    "Portfolio",
    `A prototype of a website for a start-up company providing media services.The site makes extensive use of cards arranged by Grid together with Flexbox, and a fly-down hoverable menu, to create a tidy but responsive design.

    The client's particular brief was for a website with images and videos which looked good on mobile: although only a prototype, the performance score on Lighthouse suggest a good expectation for performance.

    Responsiveness is reasonable, but in a final version would include media queries and appropriate breakpoints to make the design equally attractive on large screens.`,
    ['responsive', 'html', 'css', 'svg', 'audio', 'video', 'grid', 'flexbox', 'accessibility', 'performance'], {
        performance: 93,
        accessibility: 91,
        bestpractices: 87,
        seo: 81,
...
let dashboard = new Project("dashboard", "Dashboard",
...
<g>
   <g>
    ...
    The dashboard includes a unique 'name' search engine that I wrote from scratch in Javascript rather than borrow from a library - which was a rich, if slow, learning experience!
    
    The design makes use of Charts.js for interactive data views, and is responsive and accessible.
    
    Features include 'Read More.. Read less..' buttons for messages, and Local Storage save/clear functions.`,
    ['performance', 'responsive', 'svg', 'html', 'css', 'javascript', 'sass', 'grid', 'flexbox', 'local', 'storage', 'cookies', 'charts', 'search', ], {
        performance: 95,
        accessibility: 85,
        bestpractices: 93,
        seo: 83
...
let gallery = new Project("gallery", "Portfolio",
    ...
    "Gallery",
    `I enjoyed building this interactive, accessible, fully responsive project. SASS was my bedrock!

    The gallery relies upon Grid plus Flexbox and .baguetteBox.js library for its fluidity. Extensive use is also made of special CSS transitions and keyframe animations. The lightbox feature provides full-screen options, and integrated navigation.
    
    Like other projects, the search functionality includes a filter, that I wrote from scratch in Javascript. Building my own search functions is, I believe, making me a better developer! It's certainly a learning curve.
   `,
    ['responsive', 'image', 'html', 'css', 'javascript', 'photo', 'grid', 'flexbox', 'sass', 'lightbox', 'modal', 'search', 'gallery'], {
        performance: 93,
        accessibility: 84,
        bestpractices: 86,
        seo: 100
...
let firstwebsite = new Project("firstwebsite", "Portfolio",
    ...
    `,
    "Portfolio",
    `My first ever attempt at a fully responsive website! I'm not too ashamed ;)

    The design is relatively simple, relying upon cards arranged with Flexbox to create responsive pages. As with many of my projects, I reveal a dislike of 'vanilla' landing pages. This site relies upon CSS transitions to create some basic effects, which are intended to make the landing page both stand out, and be personal.
...
let forms = new Project("forms", "Custom forms",
    ...
    Some of the effects, using CSS transitions, are slightly overdone - for demonstration purposes - but I enjoy playing with the delicate balance between accessibility and aesthetics.
    
    In any case, a Lighthouse score of 100% for accessibility was the satisfied target, and the form scales well with media queries.`,
    ['responsive', 'fonts', 'transitions', 'html', 'css', 'form', 'media query', 'flexbox', 'accessibility'], {
        performance: 96,
        accessibility: 100,
        bestpractices: 93,
        seo: 100
...
let boxingclever = new Project("boxingclever", "Simple game",
...
<g>
    ...
    Winning is ridiculously easy in this game (don't be too embarrassed if you lose after I've said that - even the greatest hit the canvas occasionally), because the phrases are so long!
    
    Nevertheless, the core objectives in this project were to showcase my progress with Javascript, and DOM layout. I even quite like the slightly cheesy, 80s style - a concept that I borrowed for BAACODE.
    
    The layout relies upon Grid, Flexbox, and some Javascript manipulation.`,
    ['responsive', 'html', 'css', 'javascript', 'game', 'flexbox', 'svg', 'DOM', 'grid'], {
        performance: 99,
        accessibility: 83,
        bestpractices: 100,
        seo: 92
...
//--------------------------------------------------------------
// Manifest modal displays (1 = all projects, 2 = project detail)
//--------------------------------------------------------------


function displayModal() {
    modalNumber = 1;
    // hide next/previous buttons and reveal modal window
    modalNext.classList.add("hidden");
    modalPrevious.classList.add("hidden");
    overlay.classList.remove("hidden");

    //define modal HTML frame
    const modalHTML = `
    <h1 class="title-bg">it's not all fun and games...</h1>
    <input class="searchBox" onfocus = "searchFunction()" type="text" name="search" id="search" placeholder="Project techniques, e.g. 'Javascript', 'Grid'">
    ...
    projectsCollection.forEach(project => {
        ...
        let iconsContainer = document.querySelector('.modalProjectContent');
        //build the elements of each project detail

        let projectDiv = document.createElement("div");
        projectDiv.setAttribute("class", "modalProjectIcon modalProjectIconHover");

        //add short description
        let iconDescriptionText = document.createElement("h4");
        iconDescriptionText.setAttribute("class", "shortDescription");
        iconDescriptionText.innerHTML = project.description;
        ...
        projectDiv.append(iconLink);
        iconsContainer.append(projectDiv);
    });
}

//-------------------------------------------------------
// modal navigation buttons' functionality
//-------------------------------------------------------
...
modal.addEventListener('click', e => {
    ...
    } else if (e.target === modalNext && modalNumber === 2 && projectIndexTracker < projectsCollection.length - 1) {
        formatDetailedProject(projectsCollection[projectIndexTracker + 1]);
    } else if (e.target === modalPrevious && modalNumber === 2 && projectIndexTracker > 0) {
        formatDetailedProject(projectsCollection[projectIndexTracker - 1]);
    }
...
//-------------------------------------------------------
// provide modal views for project selection + navigation
//-------------------------------------------------------

let displayProjectInModal = function (target) {
    modalNumber = 2;
    // show modal navigation next and previous
    modalNext.classList.remove("hidden");
    modalPrevious.classList.remove("hidden");

    // iterate through collection of all projects and display target project detail
    // create selected project index tracker for modal next and previous functions
    projectsCollection.forEach(project => {
        if (project.projectname === target) {
            projectIndexTracker = projectsCollection.indexOf(project);
            formatDetailedProject(project);
        }
    });
...
let formatDetailedProject = function (project) {
    selectedProject = project;
    projectIndexTracker = projectsCollection.indexOf(selectedProject);
    //basic container
    let extendedProjectDiv = document.createElement("div");
    extendedProjectDiv.setAttribute("class", "extendedModal");
    extendedProjectDiv.setAttribute("style", "color:" + selectedProject.colorCode);

    let extendedProjectTitle = document.createElement("div");
    extendedProjectTitle.setAttribute("class", "detailedModalTitle");
    extendedProjectTitle.innerHTML = selectedProject.projectname;

    extendedProjectDiv.appendChild(extendedProjectTitle);
    ...
    toolTipContainer.setAttribute("background-color", "white");

    let toolTipTextPages = document.createElement("a");
    toolTipTextPages.textContent = "URL";
    toolTipTextPages.setAttribute("class", "toolTipTextPages");
    toolTipTextPages.setAttribute("style", "border: solid 1px " + selectedProject.colorCode + ";color:" + selectedProject.colorCode);
    toolTipTextPages.setAttribute("href", selectedProject.url);
    toolTipTextPages.setAttribute("target", "_blank");
    toolTipContainer.appendChild(toolTipTextPages);

    let toolTipTextRepo = document.createElement("a");
    toolTipTextRepo.textContent = "Repo";
    toolTipTextRepo.setAttribute("class", "toolTipTextRepo");
    toolTipTextRepo.setAttribute("style", "border: solid 1px " + selectedProject.colorCode + ";color:" + selectedProject.colorCode);
    toolTipTextRepo.setAttribute("href", selectedProject.repo);
    toolTipTextRepo.setAttribute("target", "_blank");
    toolTipContainer.appendChild(toolTipTextRepo);

    //add Lighthouse graphics 
    ...
    let lighthouseGraphicContainer = document.createElement("div");
    lighthouseGraphicContainer.setAttribute("class", "lighthouseGraphicContainer");
    let lighthousePerformance = document.createElement("span");
    lighthousePerformance.setAttribute("class", "score scorePerf");
    lighthousePerformance.textContent = selectedProject.lighthouse.performance;
    let lighthouseAccessibility = document.createElement("span");
    lighthouseAccessibility.setAttribute("class", "score scoreAcce");
    lighthouseAccessibility.textContent = selectedProject.lighthouse.accessibility;
    let lighthouseBestpractices = document.createElement("span");
    lighthouseBestpractices.setAttribute("class", "score scoreBest");
    lighthouseBestpractices.textContent = selectedProject.lighthouse.bestpractices;
    let lighthouseSEO = document.createElement("span");
    lighthouseSEO.setAttribute("class", "score scoreSEO");
    lighthouseSEO.textContent = selectedProject.lighthouse.seo;

    lighthouseGraphicContainer.append(lighthousePerformance, lighthouseAccessibility, lighthouseBestpractices, lighthouseSEO);
    lighthouseContainer.appendChild(lighthouseGraphicContainer);

    //assemble the parts
    extendedProjectDiv.append(toolTipContainer, descriptionText, lighthouseContainer);

    //reset the modal contents and add project detail
    modalInner.innerHTML = "";
    modalInner.appendChild(extendedProjectDiv);

    // add color-coded scores
    let entryValues = Object.values(selectedProject.lighthouse),
        colorScore = document.querySelectorAll(".score"),
        scoreColor;
    for (let i = 0; i < entryValues.length; i++) {
        let value = entryValues[i];

        if (value >= 90) {
            scoreColor = "rgb(0, 128,0)";
        } else if (value < 90 && value >= 80) {
            scoreColor = "rgb(255, 165, 0)";
        } else {
            scoreColor = "rgb(255, 0, 0, 0.8)";
        }
        colorScore[i].setAttribute("style", "color:" + scoreColor + ";border: solid 2px " + scoreColor);
    }
...
let searchFunction = function () {   
    ...
    function searchKeywords(value) {
        ...
        // reset modal projects display
        displayMatches.length = 0;


        projectsCollection.forEach(project => {
            if (project.techniques.includes(value)) {
                displayMatches.push(project);

                projectsCollection.forEach(project => {
                    let projectID = project.projectname;
                    let projectIDIcon = document.getElementById(projectID);
                    let projectIDparent = projectIDIcon.parentElement;
                    projectIDIcon.classList.add("hidden");
                    projectIDparent.classList.remove("modalProjectIconHover");

                    displayMatches.forEach(displayProject => {
                        let projectID = displayProject.projectname;
                        let projectIDIcon = document.getElementById(projectID);
                        let projectIDparent = projectIDIcon.parentElement;
                        projectIDIcon.classList.remove("hidden");
                        projectIDparent.classList.add("modalProjectIconHover");
                    });
                });
